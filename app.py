...
# 2. Initialize Groq LLM and Embeddings Model
llm = ChatGroq(api_key=api_key, model_name="llama-3.1-8b-instant")
embeddings = GroqEmbeddings(api_key=api_key, model_name="nomic-embed-text-v1.5")
...
if uploaded_file:
    ...
    if "vector_store" not in st.session_state:
        with st.spinner("Processing document... Extracting text and generating embeddings..."):
            ...
            # Split text into small chunks (helps keep relevant context close together)
            text_splitter = RecursiveCharacterTextSplitter(chunk_size=1000, chunk_overlap=200)
            final_documents = text_splitter.split_documents(docs)
            
            # Create FAISS Vector Store entirely in-memory using Groq embeddings
    ...
    if user_query:
        if "vector_store" in st.session_state:
            with st.spinner("Searching document for answers..."):
                # Define a prompt template that strictly tells the LLM to use the source text
                system_prompt = (
                ...
                # Optional portfolio flex: show the text chunks that were retrieved
